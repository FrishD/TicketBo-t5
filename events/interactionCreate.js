    ...
    async function createTicket(ticketType, reason) {
      ...
      client.guilds.cache.get(client.config.guildId).channels.create({
        ...
        .setDescription(
          ...
          .replace('REASON', reason))
        .setFooter({
          text: "Made With ❤️ By Frish#7615" + client.embeds.ticketOpened.footer.text.replace("Made With ❤️ By Frish#7615", ""), // Please respect the LICENSE :D
...
//reason whene ticket open-check if he is in the select menu of the categoris open
...
//reason whene ticket open-check if in the config file(Config.jsonc) ask reason is "true"
...
//reason whene ticket open-typing the reason if there is in the ticket panel(Whene ticket open)
...
//commends - remove user, check if the commend has been actived
